...
cameraZoom = 1.5; // Camera zoom value
...
function crateInfo() {
    $(".box").fadeOut(function() {
        ...
        $(".star-count").text(myObject[position].starCount);

        if (myObject[position].homepage) {
            $(".demo-button").attr("href", myObject[position].homepage);
            $(".demo-button").css("display", "flex");
        }

        $(".github-button").attr("href", myObject[position].url);
...
async function getGithubInformation(username) {
    const repositorys = [];
